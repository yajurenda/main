...
const themeToggle = $("theme-toggle"), longpressToggle = $("longpress-toggle");
...
function renderShop(){
  ...
  items.forEach(item=>{
    ...
    const desc = item.type==="auto" ? `※秒間+${item.effect}`
               ...
               : `※${(item.duration||30000)/1000}秒 1クリック×${item.effect}`;
    ...
    attachBuyButton(btn, item);
    shopList.appendChild(li);
  });
}

function attachBuyButton(btn, item){
  btn.replaceWith(btn.cloneNode(true)); // 二重イベント防止
  const newBtn = shopList.querySelector(`button[data-id="${item.id}"]`);
  if(longpressMode){
    let interval;
    newBtn.addEventListener("mousedown", ()=>{
      interval = setInterval(()=>buyItem(item.id),200);
    });
    newBtn.addEventListener("mouseup", ()=>clearInterval(interval));
    newBtn.addEventListener("mouseleave", ()=>clearInterval(interval));
  } else {
    newBtn.addEventListener("click", ()=>buyItem(item.id));
  }
}
...
themeToggle.addEventListener("click", () => {
  ...
  makeToast(`テーマ: ${document.body.classList.contains("dark") ? "ダーク" : "ライト"}`);
});

/* ========== 長押しモード切替 ========== */
longpressToggle.addEventListener("click", () => {
  longpressMode = !longpressMode;
  makeToast(`長押し購入モード: ${longpressMode ? "ON" : "OFF"}`);
  longpressToggle.classList.toggle("active", longpressMode);
  renderShop(); // 再描画してボタン動作を反映
});
